        ...
        switch (ch)
        {
            case KEY_RESIZE: {
                ...
                for( ; output[writtenIndex] != '\0' ; ++writtenIndex) {
                    int color_pair = 1;
                    if(state[writtenIndex] == 'c') color_pair = 2;
                    if(state[writtenIndex] == 'i') color_pair = 3;
            ...
            case KEY_BEG: case KEY_BREAK: case KEY_ENTER: case KEY_CLOSE: case KEY_COMMAND: case KEY_SCOMMAND: case KEY_HOME: case KEY_SHOME: case KEY_END: case KEY_SEND: case KEY_NEXT: case KEY_CATAB: case KEY_FIND: case KEY_HELP: case KEY_SHELP: case KEY_MARK: case KEY_IC: case KEY_IL: case KEY_DC: case KEY_DL: case KEY_PPAGE: case KEY_NPAGE: case KEY_UP: case KEY_LEFT: case KEY_SLEFT: case KEY_DOWN: case KEY_PREVIOUS: case KEY_SPREVIOUS: case KEY_RIGHT: case KEY_SRIGHT: case KEY_EXIT: case KEY_SEXIT: case ctrl('a'): case ctrl('b'): case ctrl('d'): case ctrl('e'): case ctrl('f'): case ctrl('g'): case ctrl('h'): case ctrl('i'): case ctrl('j'): case ctrl('k'): case ctrl('l'): case ctrl('m'): case ctrl('n'): case ctrl('o'): case ctrl('p'): case ctrl('q'): case ctrl('r'): case ctrl('s'): case ctrl('t'): case ctrl('u'): case ctrl('v'): case ctrl('w'): case ctrl('x'): case ctrl('y'): case ctrl('z'): case ctrl('`'): case ctrl('='): case ctrl('['): case ctrl('\\'): case KEY_F(1): case KEY_F(2): case KEY_F(3): case KEY_F(4): case KEY_F(5): case KEY_F(6): case KEY_F(7): case KEY_F(8): case KEY_F(9): case KEY_F(10): case KEY_F(11): case KEY_F(12):
                flag = false;
                break;
            default: {
                output[index] = ch;
...
int result(int* posRow, int* correct, int* wrong) {
    float accuracy = *correct + *wrong ? (float)((*correct * 100) / (*correct + *wrong)) : 0.0f;
    std::string result = "Correct characters: " + std::to_string(*correct) + " | Wrong characters: " + std::to_string(*wrong) + " | Accuracy: " + std::to_string((int) accuracy);
    int linesreq = (result.size() / COLS) + 1;    
    WINDOW* w = newwin(linesreq + 1, COLS, LINES - linesreq - 1, 0);
    wclear(w);
    mvwprintw(w, 0, 0, result.c_str());
...
int main()
{
    ...
    do {
        ...
        retry = result(&posRow, &correct_char, &wrong_char);
